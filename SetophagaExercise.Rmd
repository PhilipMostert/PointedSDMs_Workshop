...
The USAboundaries package is no longer on CRAN, so we have to download it from GitHub.
...
We can download eBird datq with the *spocc* package [@spocc] (you can laso use it to download data from places like *GBIF*, *Vernet*, *BISON* and *iNaturalist*, but eBird is enough for now). These data were obtained using the following script:
...
We also want the BBS and BBA data. For convenience, these are already in the PointedSDMs package, so we can download them directly:
...
Our covariate data are rasters, and we have them in our GitHub repository. So we can download tehm like this:
...
# Formatting the data for PointedSDMs

Now we have the data, we need to format it so that PointedSDMs can use it. This means wrangling it into something INLA will like.
...
Designing a good mesh is a dark art, that we are not sure anyone really understands. Fortunately Pennsylvania is a simple shape, so the mesh is not too complicated, but thanks to Slartibartfast's efforts, it is more difficult to gert a good meshcointrios like Norway are 
...
caerulescensModel <- intModel(caerulescensData, Boundary = PA,
                              Coordinates = c("X", "Y"),
