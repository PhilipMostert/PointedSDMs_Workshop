...
- share everything: $\varepsilon_i(\bf{s}) = \varepsilon(\bf{s})$. Same random field across all species
- share hyper-parameters: $\sigma_i^2 = \sigma^2$. Different random field, but shared hyperparameters
- share nothing: $\varepsilon_i(\bf{s})$ and $\sigma_i^2$ all different
...
In multi-species models, we may want to allow for species-specific responses to environmental variables  In our case, we know that our three Setophaga species have different associations with coniferous forests: 
- *Setophaga caerulescens* prefers deciduous forests
- *Setophaga fusca* is strongly associated with mature coniferous and mixed forests
- *Setophaga magnolia* favors coniferous and mixed forests
