...
If we want to model the observation process for a data set, we can do that by changing the formula for the dataset. For example, we might think that birders in Pennsylvania don't like heights (if they did, they would be in Colorado, after all), and add elevation as a covariate:
...
caerulescensModel$spatialBlock(k = 4, rows_cols = c(2, 2), plot = TRUE) + theme_bw()
...
caerulescens_no_fields$spatialBlock(k = 4, rows_cols = c(2, 2), plot = TRUE) + theme_bw()
